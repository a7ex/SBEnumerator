    ...
    static func processCLIArgs(cliParams: [String], mapping: ((String) -> String) = { return $0 }) -> [String: Any] {
        ...
        for (index, thisParam) in cliParams.enumerated() {
            ...
            if thisParam.hasPrefix("--") {
                ...
                currentParamName = String(thisParam.suffix(from: thisParam.index(thisParam.startIndex, offsetBy: 2)))
            } else if thisParam == "--" {
                endparsing = true
            } else if thisParam == "-" {
                endparsing = true
            } else if thisParam.hasPrefix("-") {
                let switches = thisParam.suffix(from: thisParam.index(thisParam.startIndex, offsetBy: 1))

                // check for the special case of "-" starting a negative number parameter:
                let num = switches.prefix(upTo: switches.index(switches.startIndex, offsetBy: 1))
                if "0123456789".contains(num) {
                    ...
                    for (index, thisChar) in switches.enumerated() {
